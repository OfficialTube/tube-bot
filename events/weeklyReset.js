    ...
    cron.schedule(
        ...
        async () => {
            try {
                ...
                // 1️⃣ Fetch all members who CURRENTLY have the Yapper role and remove it
                const yapperRole = await guild.roles.fetch(YAPPER_ROLE_ID);
                if (yapperRole) {
                    for (const [_, member] of yapperRole.members) {
                        await member.roles.remove(YAPPER_ROLE_ID).catch(() => {});
                    }
                }

                // 2️⃣ Fetch top active users (MODIFIED: Must have MORE than 70 weekly XP)
                const allUsers = await User.find({ weeklyxp: { $gt: 70 } }).sort({ weeklyxp: -1 });

                // 3️⃣ Fetch only relevant members to avoid API timeouts
                ...
                // 4️⃣ Determine top 3 users (excluding mods)
                const topUsers = [];
                for (const userData of allUsers) {
                ...
                if (channel) {
                    if (rewardInfo.length) {
                        ...
                        await channel.send('No eligible yappers this week (nobody passed 70 XP).');
                    }
                }
