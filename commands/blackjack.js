...
module.exports = {
    ...
    async execute(interaction) {
        ...
        startBlackjack(interaction.channel, user);
    }
};

async function startBlackjack(channel, user) {
    ...
    // Send a new message for each game
    const gameMessage = await channel.send({ embeds: [embed], components: [row] });
    ...
    collector.on("collect", async i => {
        if (i.customId === "hit") {
            ...
            if (playerTotal===21 && playerHand.length===2 && (dealerHand.length !== 2 || dealerTotal !== 21)) {
                ...
                user.money += money;
                user.losses++; user.rounds++; user.moneyLost += 10;
                user.moneyNet = user.moneyGained - user.moneyLost;
                user.points += points;
                streak=0;
                await user.save();
                ...
                user.money += money; user.wins++; user.rounds++;
                user.moneyGained += money;
                user.moneyNet = user.moneyGained - user.moneyLost;
                streak=user.streakCurrent;
                if(user.streakCurrent>user.streakBest) user.streakBest=user.streakCurrent;
                ...
                user.money += money; user.losses++; user.rounds++; user.moneyLost+=10;
                user.moneyNet = user.moneyGained - user.moneyLost;
                streak=0;
                user.points--;
                await user.save();
            ...
            playAgainCollector.on("collect", async i => {
                ...
                await i.update({ components: [] }); // Remove Play Again button
                startBlackjack(channel, user); // Start a fresh new game message
            });
        }
    });
}
