...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            return interaction.editReply({
                content: `**__${targetUser.tag.replace(/([*_`~|\\])/g, '\\$1')}'s Slots Stats__**\nMoney Earned: ${formatMoney(user.moneyEarnedSlots)}\n${slotsMoneyDisplay}\nRounds: ${formatNumber(user.roundsSlots)}\n\nDoubles:\n:one: - ${formatNumber(user.double1)}\n:two: - ${formatNumber(user.double2)}\n:three: - ${formatNumber(user.double3)}\n:four: - ${formatNumber(user.double4)}\n:five: - ${formatNumber(user.double5)}\n:six: - ${formatNumber(user.double6)}\n:seven: - ${formatNumber(user.double7)}\n:eight: - ${formatNumber(user.double8)}\n:nine: - ${formatNumber(user.double9)}\n\nTriples:\n:one: - ${formatNumber(user.triple1)}\n:two: - ${formatNumber(user.triple2)}\n:three: - ${formatNumber(user.triple3)}\n:four: - ${formatNumber(user.triple4)}\n:five: - ${formatNumber(user.triple5)}\n:six: - ${formatNumber(user.triple6)}\n:seven: - ${formatNumber(user.triple7)}\n:eight: - ${formatNumber(user.triple8)}\n:nine: - ${formatNumber(user.triple9)}\n\nMoney Earned: ${formatMoney(user.moneyEarnedSlots)}\nMoney Spent: ${formatMoney(user.moneySpentSlots)}\nNet Money: ${formatMoneyNet(user.moneyEarnedSlots, user.moneySpentSlots)}\n\nMax Money Won: ${formatMoney(user.maxWon)}`
            });
